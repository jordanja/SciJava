...
public class BoxPlot extends Plot{
	...
	public void drawPlot(Graphics2D g, BoxChartAxis axis, HashMap<Object, Object> data, String[] xDataOrdered, String typeOfData, XYChartMeasurements cm) {
		...
		if (typeOfData == "singleCatagory") {
			...
			drawBar(g,xBoxStart, yBoxStart, boxWidth, boxHeight, xCenterOfBox, yMin, yMax, yMedian , this.barColor, this.outlineColor);
			
		} else if (typeOfData == "multipleCatagoriesAndNoHueValue") {
			int widthOfbar = (int)(singlularClusterBarWidthPercentage * cm.getPlotWidth() / numberOfXTicks);
			...
			for (String xCategory : xDataOrdered) {
				...
				HashMap<Object, Object> map = (HashMap<Object, Object>) data.get(xCategory);

				double[] yTicks = Arrays.stream(axis.getyTicks()).mapToDouble(Double::parseDouble).toArray();
				
				double min = (Double) map.get("Min");
				double Q1 = (Double) map.get("Q1");
				double Q2 = (Double) map.get("Q2");
				double Q3 = (Double) map.get("Q3");
				...
				int yMax = yTickNumToPlotY(max, yTicks, cm);
				int yMedian = yTickNumToPlotY(Q2, yTicks, cm);
				drawBar(g,xBoxStart, yBoxStart, boxWidth, boxHeight, xCenterOfBox, yMin, yMax, yMedian , fillColor, this.outlineColor);
			...
			int xCatagoryCount = 0;
			for (String xCategory: xDataOrdered) {				
				HashMap<Object, Object> category = (HashMap<Object, Object>) data.get(xCategory);
				...
				for (Object colorCode: category.keySet()) {
					...
					HashMap<Object, Object> map = (HashMap<Object, Object>) category.get(colorCode);
...
//					System.out.println("min: " + min + "\n" + "Q1: " + Q1 + "\n" + "Q2: " + Q2 + "\n" + "Q3: " + Q3 + "\n" + "max: " + max);
					...
					int xBoxStart = positionAtBarsStart + ((widthOfColorCodeBar + multipleBarPixelSpacing) * colorCodeCount);
					int yBoxStart = yTickNumToPlotY(Q1, yTicks, cm);
	...
	private void drawBar(Graphics2D g, int xBoxStart, int yBoxStart, int boxWidth, int boxHeight, int xCenterOfBox, int yMin, int yMax, int yMedian, Color fillColor, Color outlineColor) {
